// Feedlot inventory — "📦 Inventory pill" body on the Feedlot page.
//
// Mirrors the Dairy inventory card pattern: items grouped by category
// (Bedding · Equipment · Veterinary · Feed) with Item · Qty · Location ·
// Condition · Actions columns. Quantities are editable inline via a
// small dialog.
...
// ----- model (kept local to the file; the schema is simple enough that
// a separate `feedlot.dart` model file would be overkill) -----
class FeedlotInventoryItem {
  FeedlotInventoryItem({
    required this.id,
    required this.name,
  ...
  factory FeedlotInventoryItem.fromJson(Map<String, dynamic> j) {
...
class FeedlotInventoryCard extends StatefulWidget {
  const FeedlotInventoryCard({super.key});

  @override
  State<FeedlotInventoryCard> createState() => _FeedlotInventoryCardState();
}

class _FeedlotInventoryCardState extends State<FeedlotInventoryCard> {
  static const _primary = Color(0xFF27500A);
  static const _txt3 = Color(0xFF99A39B);

  // Render order — categories the seed creates, then anything new.
  static const _categoryOrder = ['Bedding', 'Equipment', 'Veterinary', 'Feed'];

  late Future<List<FeedlotInventoryItem>> _future;
  ...
  Future<List<FeedlotInventoryItem>> _load() async {
    final raw = await ApiService.getFeedlotInventory();
    return raw
        .whereType<Map>()
        .map((m) => FeedlotInventoryItem.fromJson(m.cast<String, dynamic>()))
        .toList();
  }
  ...
  Future<void> _editQuantity(FeedlotInventoryItem item) async {
    final controller =
    ...
    try {
      await ApiService.updateFeedlotInventoryItem(
        item.id,
        {'quantity': result},
      );
      await _reload();
  ...
  // qty == 0 → Out · qty < 5 → Low · else use stored condition (Good).
  String _displayCondition(FeedlotInventoryItem item) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: FutureBuilder<List<FeedlotInventoryItem>>(
        future: _future,
        builder: (context, snap) {
          ...
          final items = snap.data ?? const <FeedlotInventoryItem>[];
          final groups = <String, List<FeedlotInventoryItem>>{};
...
class _CategoryGroup extends StatelessWidget {
  ...
  final String category;
  final List<FeedlotInventoryItem> items;
  final String Function(double) formatQty;
  final String Function(FeedlotInventoryItem) displayCondition;
  final ValueChanged<FeedlotInventoryItem> onEdit;
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Text(
        'No inventory items recorded for the feedlot yet.',
