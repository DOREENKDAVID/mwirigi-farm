// Piggery inventory — body for the "📦 Inventory" pill on the Piggery
// page. Mirrors FeedlotInventoryCard / DairyInventoryCard structure.

import 'package:flutter/material.dart';

import '../../core/service/api_service.dart';

class PiggeryInventoryItemModel {
  PiggeryInventoryItemModel({
    required this.id,
    required this.name,
  ...
  factory PiggeryInventoryItemModel.fromJson(Map<String, dynamic> j) {
    ...
    return PiggeryInventoryItemModel(
      id: (j['id'] ?? '').toString(),
...
class PiggeryInventoryCard extends StatefulWidget {
  const PiggeryInventoryCard({super.key});

  @override
  State<PiggeryInventoryCard> createState() => _PiggeryInventoryCardState();
}

class _PiggeryInventoryCardState extends State<PiggeryInventoryCard> {
  ...
  late Future<List<PiggeryInventoryItemModel>> _future;
  ...
  Future<List<PiggeryInventoryItemModel>> _load() async {
    final raw = await ApiService.getPiggeryInventory();
    return raw
        .whereType<Map>()
        .map((m) =>
            PiggeryInventoryItemModel.fromJson(m.cast<String, dynamic>()))
        .toList();
  }
  ...
  Future<void> _editQuantity(PiggeryInventoryItemModel item) async {
    ...
    try {
      await ApiService.updatePiggeryInventoryItem(
        item.id,
        {'quantity': result},
      );
      await _reload();
  ...
  String _displayCondition(PiggeryInventoryItemModel item) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: FutureBuilder<List<PiggeryInventoryItemModel>>(
        future: _future,
        builder: (context, snap) {
          ...
          final items = snap.data ?? const <PiggeryInventoryItemModel>[];
          final groups = <String, List<PiggeryInventoryItemModel>>{};
...
class _CategoryGroup extends StatelessWidget {
  ...
  final String category;
  final List<PiggeryInventoryItemModel> items;
  final String Function(double) formatQty;
  final String Function(PiggeryInventoryItemModel) displayCondition;
  final ValueChanged<PiggeryInventoryItemModel> onEdit;
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Text(
        'No inventory items recorded for the piggery yet.',
